...
function getOptimalDemandMultiplier(actualMix: Record<string, number>): number {
  const actualWeightedCost = ENERGY_TYPES.reduce(
    (s, t) => s + (actualMix[`${t}_pct`] / 100) * OPTIMAL_COSTS[t],
    0,
  );
  const optimalWeightedCost = ENERGY_TYPES.reduce(
    (s, t) => s + (OPTIMAL_MIX[t] / 100) * OPTIMAL_COSTS[t],
    0,
  );
...
export function generateDayData(dateStr: string): DayData {
  ...
  const hours: HourData[] = Array.from({ length: 24 }, (_, h) => {
    ...
    const solarAvail =
      h >= 6 && h <= 20 ? Math.sin(((h - 6) / 14) * Math.PI) : 0;
    ...
    const peakMult =
      isMorningPeak || isEveningPeak ? 1.35 + rng() * 0.45 : 1.0 + rng() * 0.18;
  ...
  const avgMix = Object.fromEntries(
    ENERGY_TYPES.map((t) => [
      t,
      Math.round(
        (hours.reduce((a, h) => a + (h[`${t}_pct`] as number), 0) / 24) * 10,
      ) / 10,
    ]),
  ...
  return {
    ...
    weather: {
      ...
      solarIrradiance: Math.round(rng() * 200 + 50),
    },
  };
}
