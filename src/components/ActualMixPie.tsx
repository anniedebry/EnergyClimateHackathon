...
export default function ActualMixPie({ avgMix }: ActualMixPieProps) {
  const pieData = ENERGY_TYPES.map(t => ({ name: ENERGY_LABELS[t], value: avgMix[t], key: t }));

  return (
    <ChartCard label="Today's Energy Mix" subtitle="Actual average source distribution">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: `6px ${spacing.md}px`, marginTop: spacing.sm }}>
        {ENERGY_TYPES.map(t => (
          <div key={t} style={{ display: "flex", alignItems: "center", gap: spacing.xs }}>
            ...
            <span style={{ fontSize: font.sm, color: colors.textSecondary, marginLeft: "auto", fontWeight: 500 }}>{avgMix[t]}%</span>
          </div>
        ))}
      </div>
    </ChartCard>
